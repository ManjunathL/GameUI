...
public class ProposalDataProvider {
    ...
    public List<ProposalVersion> getProposalVersionafterProduction(int proposalId) {
        JSONArray jsonArray = dataProviderMode.getResourceArray("proposal/version/afterproduction", new HashMap<String, String>() {
            {
                put("proposalId", proposalId + "");
            }
        });
   ...
   /* public String getProdSpecFile(ProductAndAddonSelection productAndAddonSelection) {
    ...
    public boolean updateCrmPrice(SendToCRM sendToCRM)
    {
        try {
            ...
            JSONResource jsonObject = dataProviderMode.postResourceWithUrlForCrm(baseCrmUrl, sendToCRM.getOpportunity_name(),final_amount,estimated_project_cost,sendToCRM.getQuotation_number_c(),booking_order_value_cost,sendToCRM.getProposal_link_c(),sendToCRM.getPresales_user_email(),String.valueOf(sendToCRM.getNo_of_working_days()),dso_date);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public PriceMaster getHingeRateDetails(String rateId, Date priceDate, String city)
    {
        JSONObject jsonObject = dataProviderMode.getResource("proposal/hingedetails", new HashMap<String, String>(){
            {
                put("rateId", rateId + "");
                put("priceDate", priceDate + "");
                put("city",city + "");
