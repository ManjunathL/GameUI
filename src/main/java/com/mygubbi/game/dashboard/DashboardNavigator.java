...
import com.mygubbi.game.dashboard.event.DashboardEvent.BrowserResizeEvent;
import com.mygubbi.game.dashboard.event.DashboardEvent.CloseOpenWindowsEvent;
...
public class DashboardNavigator extends Navigator {
    ...
    private static final DashboardViewType.ViewType ERROR_VIEW = DashboardViewType.PROPOSALS.getViewType();
    ...
    public DashboardNavigator(final ComponentContainer container) {
        ...
        if (TRACKER_ID != null && host.endsWith("game.mygubbi.com")) {
    ...
    private void initGATracker(final String trackerId) {
        tracker = new GoogleAnalyticsTracker(trackerId, "game.mygubbi.com");
    ...
    private void initViewProviders() {
        ...
        for (final DashboardViewType viewType : DashboardViewType.values()) {
            addViewProvider(viewType.getViewType());

            for (DashboardViewType.ViewType viewType1 : viewType.getViewType().getSubViewTypes()) {
                addViewProvider(viewType1);
            }
        }
    ...
    private void addViewProvider(final DashboardViewType.ViewType viewType) {
